...
import urllib3, io, requests, os, urllib.request
...
class main:
	...
	def update_status(self, update_status, url, saveas):
		...
		while True:
			...
			try:
				if length:
					print(update_status.replace('REPLACE', str(int(size/1048576))).replace('TOTALSIZE', str(int(length/1048576))), end='\r')
			except:
				print(update_status.replace('REPLACE', 'null').replace('TOTALSIZE', 'null', end='\r'))
		try:
			print(update_status.replace('REPLACE', str(int(length/1048576))).replace('TOTALSIZE', str(int(length/1048576))))
		except:
	...
	def blauncher(self):
		...
		if not os.path.exists(f'{vars.blauncher_dir}\\launcher\\lc.jar'):
			self.update_status(f'{vars.informative} Downloading Lunar client... (REPLACE/TOTALSIZE MB)', 'https://www.dropbox.com/s/11xrh03k9f1wj02/LC.jar?dl=1', f'{vars.blauncher_dir}\\launcher\\lc.jar')
		else:
			if self.check_md5sum(f'{vars.blauncher_dir}\\launcher\\lc.jar') == '65627d3205d9d25f9c73bb16bc35b054':
		...
		if not os.path.exists(f'{vars.blauncher_dir}\\launcher\\dc.jar'):
			self.update_status(f'{vars.informative} Downloading Doge client... (REPLACE/TOTALSIZE MB)', 'https://www.dropbox.com/s/660uegvhlqnylg8/dc.jar?dl=1', f'{vars.blauncher_dir}\\launcher\\dc.jar')
		else:
			self.check_md5sum(f'{vars.blauncher_dir}\\launcher\\dc.jar')
			print(f'{vars.error} Doge client already installed!')
		print('')

		if not os.path.exists(f'{vars.blauncher_dir}\\libraries'):
			self.update_status(f'{vars.informative} Downloading libraries... (REPLACE/TOTALSIZE MB)', 'https://www.dropbox.com/s/2fy50egnytp2fwu/libraries.zip?dl=1', f'{vars.blauncher_dir}\\libraries.zip')
		...
		if not os.path.exists(f'{vars.blauncher_dir}\\lib'):
			self.update_status(f'{vars.informative} Downloading client libraries... (REPLACE/TOTALSIZE MB)', 'https://www.dropbox.com/s/lfatlncck33h1pf/lib.zip?dl=1', f'{vars.blauncher_dir}\\lib.zip')
		...
		if not os.path.exists(f'{vars.blauncher_dir}\\launcher.exe'):
			with requests.Session() as s:
				...
				launcher_url = s.get('http://blauncher.atwebpages.com/latest_launcher', headers={'Pragma': 'no-cache'}, verify=False).text
				self.update_status(f'{vars.informative} Downloading Launcher... (REPLACE/TOTALSIZE MB)', launcher_url, f'{vars.blauncher_dir}\\launcher.exe')
				try:
		...
		if not os.path.exists(f'{vars.blauncher_dir}\\updater.exe'):
			with requests.Session() as s:
				urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
				updater_url = s.get('http://blauncher.atwebpages.com/latest_updater', headers={'Pragma': 'no-cache'}, verify=False).text
				self.update_status(f'{vars.informative} Downloading Updater... (REPLACE/TOTALSIZE MB)', updater_url, f'{vars.blauncher_dir}\\updater.exe')

		write_v = open(f'{vars.blauncher_dir}\\version.txt', 'w')
		with requests.Session() as s:
			latest_version = s.get('http://blauncher.atwebpages.com/latest_version', headers={'Pragma': 'no-cache'}, verify=False).text
